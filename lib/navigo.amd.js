...
function Navigo(appRoute, resolveOptions) {
  ...
  function _updateBrowserURL(context, done) {
    if ((0,_utils__WEBPACK_IMPORTED_MODULE_0__.undefinedOrTrue)(context.navigateOptions, "updateBrowserURL")) {
      ...
      if (isWindowAvailable && context.resolveOptions && context.resolveOptions.hash === true) {
        location.hash = value;
      } else if (isPushStateAvailable) {
        history[context.navigateOptions.historyAPIMethod || "pushState"](context.navigateOptions.stateObj || {}, context.navigateOptions.title || "", value // making sure that we don't have two slashes
        );
  ...
  function getCurrentLocation() {
    return pathToMatchObject((0,_utils__WEBPACK_IMPORTED_MODULE_0__.clean)(getCurrentEnvURL()).replace(new RegExp("^" + root), ""));
  }
