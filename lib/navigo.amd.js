...
function Navigo(r) {
  ...
  }, lifecycle, [_errorOut, _checkForLeaveHook]), _flushCurrent];
  ...
  function _checkForLeaveHook(context, done) {
    ...
    if (current && current[0] && current[0].route && (!context.match || current[0].route.path !== context.match.route.path)) {
      (0,_Q__WEBPACK_IMPORTED_MODULE_1__.default)([].concat(current.map(function (match) {
        return function (_, leaveLoopDone) {
          if (match.route.hooks && match.route.hooks.leave) {
            match.route.hooks.leave(function (moveForward) {
              if (typeof moveForward === "undefined" || moveForward === true) {
                leaveLoopDone();
              }
            }, match);
  ...
  function _processMatches(context, done) {
    ...
    (function nextMatch() {
      ...
      (0,_Q__WEBPACK_IMPORTED_MODULE_1__.default)([].concat(lifecycle, [function processMatchesEnd() {
        idx += 1;
        nextMatch();
      }]), _extends({}, context, {
